//Given a binary search tree, design an algorithm which creates a linked list of all the nodes at each depth(i.e if you have a tree with depth D then you will have D linked lists)
...
void createMinimalTree(Node * &head, Node *parent, int a[], int start, int end){
	if(start <= end){
		...
		createMinimalTree(head0>lchild, head, a, mid+1, end);
	}
}

vector<list<Node*>> find_level_linkedlists(Node *head){
...
void print(vector<list<Node*>> res){
	...
	for(vit = res.begin(); vis != res.end(); ++vit){
		list<Node*> li = *vit;
